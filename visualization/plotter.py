...
class Plotter(object):
    ...
    def overfitting(self, estimator, X_train, X_test, y_train, y_test, bins=50):
        ...
        plt.bar(bin_edges[:-1], hist_bkg_train, width=width,
                color='steelblue', alpha=0.5, label='clone (train)')
        ...
        plt.errorbar(bin_edges[:-1], hist_sig_test, yerr=err, fmt='o', c='r', label='acdc (test)')
        ...
        self.ax.set_title("%s: acdc (clone)\nks: %0.3f (%0.3f)\np-value: %0.3f (%0.3f)"
    ...
    ## Define ROC curve plot
    def roc_curve_cv(self, estimators, X, y, folds_included=False, n_splits=10):
        ...
        plt.xlabel('False Positive Rate')
        plt.ylabel('True Positive Rate')
        plt.title("Receiver operating characteristic ({} workflows)".format(X.shape[0]))
        leg = plt.legend(loc="lower right", frameon=True, fancybox=True, fontsize=8) # loc='best'
        leg.get_frame().set_edgecolor('w')
        frame = leg.get_frame()
        frame.set_facecolor('White')    
    ...
    ## Learning curve
    def learning_curve(self, estimator, X_train, y_train,
        ...
        train_sizes_abs, train_scores, test_scores = learning_curve(model,
                                                                    X_train, y_train,
                                                                    train_sizes=train_sizes, #np.linspace(0.1, 1.0, 10),
                                                                    cv=cv,
                                                                    scoring=scoring,
